...
APP_NAME = "Agent Lead Enrichment API"
APP_SLUG = "leadenrichapi"
DEFAULT_BASE_URL = "https://agent-lead-enrichment-api.up.railway.app"
...
FOLLOWUP_FROM_EMAIL = os.getenv("FOLLOWUP_FROM_EMAIL", "Agent Lead Enrichment API <joseph@dataweaveai.com>").strip()
...
@app.post("/api/public/demo/leak-scan")
def leak_scan_demo(payload: LeakScanRequest, request: Request) -> dict[str, Any]:
    if ENFORCE_STRICT_PAID_API:
        raise HTTPException(status_code=403, detail="Demo endpoint disabled. Paid API key required")
    ip = client_ip(request)
    check_rate_limit(f"demo-scan:{ip}", 6, API_RATE_WINDOW_SECONDS)
    return leak_scan(payload, request, require_key=True)


@app.post("/v1/leak-scan")
def leak_scan(payload: LeakScanRequest, request: Request, require_key: bool = True) -> dict[str, Any]:
    if require_key:
